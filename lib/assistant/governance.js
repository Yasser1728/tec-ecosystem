...
export class TECAssistantGovernance {
  ...
  async verifySecurityContext(userId, context) {
    try {
      ...
      // On error or timeout, allow with warning
      console.warn('Security verification failed, allowing with warning:', error.message);
      return {
        passed: true, // Allow on error to prevent blocking
        riskScore: 50,
        issues: []
      };
    }
