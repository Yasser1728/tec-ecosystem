...
export class TECAssistantGovernance {
  ...
  async processGovernedRequest(userId, message, context = {}) {
    try {
      ...
      return await Promise.race([
        this._processGovernedRequestInternal(userId, message, context),
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Governance processing timeout')), 5000)
        )
      ]);
    } catch (error) {
      console.error('Governance processing error:', error);
      // Return a basic approved response on error
      return {
        success: true,
        language: this.detectLanguage(message),
        responseType: 'advisory',
        dashboard: { type: 'decision_support', language: 'en', sections: [] },
        insights: { type: 'advisory', domains: [], recommendations: [], warnings: [] },
        governance: {
          approved: true,
          domains: ['system'],
          restrictions: []
  ...
  /**
   * Internal governance processing (can timeout)
   */
  async _processGovernedRequestInternal(userId, message, context = {}) {
   ...
   * 
   * For advisory-only assistant requests, use relaxed verification:
   * - Full Zero-Trust is enforced for transactional/financial endpoints
   * - Advisory queries are low-risk (no data modification, no financial impact)
   * - Only block if risk score is critically high (>80)
   * - Still log and return actual verification results for monitoring
   * 
   * @param {string} userId - User identifier
   * @param {Object} context - Request context
  ...
  async verifySecurityContext(userId, context) {
    try {
      ...
      // For advisory-only assistant: Only block if risk is critically high
      // Still return actual verification results for security monitoring
      const shouldBlock = verification.riskScore > 80;
      
      return {
        passed: !shouldBlock,
        riskScore: verification.riskScore || 0,
        issues: verification.results ? verification.results.filter(r => !r.passed) : [],
        relaxedForAdvisory: true,
        actualVerification: verification // Keep actual results for monitoring
      };
    } catch (error) {
      // On error or timeout, allow with warning (advisory access)
      // Use null risk score to indicate verification unavailable (not zero risk)
      console.warn('Security verification failed, allowing advisory access:', error.message);
      return {
        passed: true,
        riskScore: null, // null indicates unknown risk, not zero risk
        issues: [],
  ...
  async checkDomainGovernance(userId, targetDomains, context) {
    try {
      return await Promise.race([
        this._checkDomainGovernanceInternal(userId, targetDomains, context),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), 2000))
      ]);
    } catch (error) {
      console.warn('Domain governance check failed, allowing with warning:', error.message);
      // On error, allow all domains
      return {
        approved: true,
  ...
  async _checkDomainGovernanceInternal(userId, targetDomains, context) {
    ...
    try {
      ...
      try {
        const dashboard = await Promise.race([
          analyticsDashboard.getDashboardData(domain, {
            timeRange: '24h'
          }),
        ...
        // Dashboard not available for this domain - continue without it
        console.log(`Dashboard not available for ${domain}:`, error.message);
      }
