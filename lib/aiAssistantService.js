...
class AIAssistantService {
  ...
  async loadTemplate(templateName) {
    try {
      ...
      const sanitizedTemplateName = sanitizePathComponent(templateName, 'templateName');
      const filePath = path.join(this.templatesDir, `${sanitizedTemplateName}.json`);
      const data = await fs.readFile(filePath, 'utf8');
      return JSON.parse(data);
    } catch (error) {
  ...
  async loadModelConfig(modelName) {
    try {
      ...
      const configPath = path.resolve(this.modelsDir, `${sanitizedModelName}.config.json`);
  ...
  async exportAssistantData(assistantId, exportFormat = 'json') {
    try {
      ...
      const sanitizedAssistantId = sanitizePathComponent(assistantId, 'assistantId');
      // Security: sanitize 'exportFormat' before use in path.join to prevent path traversal
      const sanitizedFormat = sanitizePathComponent(exportFormat, 'exportFormat');
      
      const exportPath = path.join(this.baseDir, 'exports', `${sanitizedAssistantId}.${sanitizedFormat}`);
  ...
  async importAssistantData(filename) {
    try {
      ...
      const importPath = path.join(this.baseDir, 'imports', sanitizedFilename);
