...
/**
 * Get domain by route path
 * @param {string} route - The route path (e.g., "/fundx")
 * @returns {Object|null} Domain configuration or null
 */
export function getDomainByRoute(route) {
  const entry = Object.entries(domainMapping).find(
    ([_, config]) => config.route === route
  );
  return entry ? { domain: entry[0], ...entry[1] } : null;
}

/**
 * Validate if a domain is operational
 * @param {string} domain - The .pi domain name
 * @returns {boolean} Whether domain is active
 */
export function isDomainOperational(domain) {
  return domainMapping[domain]?.status === "active";
}

/**
 * Get domains requiring monthly audit
 * @returns {Array<Object>} Domains with monthly audit frequency
 */
export function getMonthlyAuditDomains() {
  return Object.entries(domainMapping)
    .filter(([_, config]) => config.governance?.auditFrequency === "monthly")
    .map(([domain, config]) => ({ domain, ...config }));
}
