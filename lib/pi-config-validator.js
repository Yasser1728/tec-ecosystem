 ...
 * @export Shared helper used across modules to detect sandbox mode
 */
...
export function validatePiConfig() {
  ...
  const placeholderPatterns = [
    /^your[_-].*[_-]here$/i,  // your_*_here or your-*-here
    /^your[_-]pi[_-]/i,        // your_pi_* or your-pi-*
    /^sandbox[_-]?$/i,         // just "sandbox"
    /^test[_-]?$/i,            // just "test"
    /^change[_-]?me/i,         // change_me or changeme
    /^replace[_-]?me/i,        // replace_me or replaceme
  ];
  ...
  if (isPlaceholder(process.env.PI_API_SECRET)) {
    warnings.push("PI_API_SECRET");
  }
  if (isPlaceholder(process.env.PI_WALLET_ADDRESS)) {
 ...
 * Validates that API key is configured for production mode
 * @param {boolean} requireApiKey - If true, throws error when API key missing in production
 * @returns {Object} API configuration
 * @throws {Error} If requireApiKey is true and API key is missing in production mode
 */
export function getPiApiConfig(requireApiKey = false) {
  const isSandbox = isSandboxMode();
  const apiKey = process.env.PI_API_KEY;

  // Validate API key for production if required
  if (requireApiKey && !isSandbox && !apiKey) {
    throw new Error("PI_API_KEY is required for production mode but not configured");
  }
