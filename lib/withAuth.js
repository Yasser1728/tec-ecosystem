    ...
    useEffect(() => {
      ...
      if (session?.user) {
        ...
        if (!hasTierLevel(userTier, requiredTier)) {
          router.push(`/upgrade?required=${requiredTier}&from=${encodeURIComponent(router.asPath)}`);
          return;
        }
...
export function useAuth(options = {}) {
  ...
  const redirectToUpgrade = (required) => {
    ...
    router.push(`/upgrade?required=${tier}&from=${encodeURIComponent(router.asPath)}`);
  };
...
export function AuthGuard({ children, requiredTier, fallback = null }) {
  const { isAuthenticated, isLoading, hasRequiredTier } = useAuth({ requiredTier });
