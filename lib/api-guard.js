...
// Cleanup interval - only in non-serverless environments
let cleanupInterval = null;
if (typeof process !== 'undefined' && process.env.NODE_ENV !== 'test') {
  // Only set up automatic cleanup if not in serverless or test environment
  const isServerless = process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME || process.env.FUNCTION_NAME;
  if (!isServerless) {
    cleanupInterval = setInterval(cleanupRateLimitStore, 10 * 60 * 1000);
  }
}

// Manual cleanup function for serverless environments
export function triggerRateLimitCleanup() {
...
function checkRequestSize(body, maxSizeBytes) {
  // Quick check for obviously large objects
  if (typeof body === 'object' && body !== null) {
    // Estimate size based on key count for efficiency
    const keyCount = Object.keys(body).length;
    if (keyCount > 1000) {
      // Too many keys, likely too large
      return false;
    }
  }
  
  // Only stringify if necessary
