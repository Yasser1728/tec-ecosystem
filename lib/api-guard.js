...
export function sanitizeInput(input) {
  ...
  // Use a more robust sanitization approach with iterative replacement
  let sanitized = input;
  let previousLength;
  
  // Remove all HTML tags completely (iterate until no more changes)
  do {
    previousLength = sanitized.length;
    sanitized = sanitized.replace(/<[^>]*>/g, '');
  } while (sanitized.length !== previousLength);
  
  // Remove any remaining angle brackets (iterate until no more changes)
  do {
    previousLength = sanitized.length;
    sanitized = sanitized.replace(/[<>]/g, '');
  } while (sanitized.length !== previousLength);
  
  // Remove javascript:, data:, and vbscript: protocols (iterate until no more changes)
  do {
    previousLength = sanitized.length;
  ...
  // Remove event handlers (iterate until no more changes)
  do {
    previousLength = sanitized.length;
