...
export function detectSuspiciousActivity(
  ...
  if (
    auditContext.recentOperations &&
    auditContext.recentOperations.length >
      SECURITY_THRESHOLDS.RAPID_OPERATIONS_COUNT
  ) {
    const recentCount = auditContext.recentOperations.filter(
      (op) =>
        new Date(op.timestamp) >
        new Date(Date.now() - SECURITY_THRESHOLDS.RAPID_OPERATIONS_WINDOW_MS),
    ).length;
  ...
  if (
    data.amount &&
    data.amount > SECURITY_THRESHOLDS.LARGE_TRANSACTION_AMOUNT
  ) {
  ...
  if (auditContext.userCreatedAt) {
    const accountAge =
      Date.now() - new Date(auditContext.userCreatedAt).getTime();

    if (
      accountAge < SECURITY_THRESHOLDS.NEW_ACCOUNT_AGE_MS &&
  ...
  try {
    ...
    const auditLogRecord = await prisma.auditLog.create({
      data: {
        ...
        rejectionReason: logEntry.approved
          ? null
          : generateRejectionReason(logEntry),
        hash: logEntry.hash,
...
function generateRejectionReason(logEntry) {
  ...
  if (!logEntry.identityCheck?.verified) {
    reasons.push(
      logEntry.identityCheck?.reason || "Identity verification failed",
    );
  }
...
function applyAuditOptions(options = {}) {
  ...
  const sanitizedLimit = Math.min(
    Math.max(
      AUDIT_PAGINATION.MIN_LIMIT,
      parseInt(limit, 10) || AUDIT_PAGINATION.DEFAULT_LIMIT,
    ),
    AUDIT_PAGINATION.MAX_LIMIT,
  );
  const sanitizedOffset = Math.max(
    AUDIT_PAGINATION.DEFAULT_OFFSET,
    parseInt(offset, 10) || AUDIT_PAGINATION.DEFAULT_OFFSET,
  );
...
export async function updateSystemIntegrity(auditResult) {
  try {
    ...
    if (recentAnomalies >= 10) {
      ...
      lockReason =
        "Automatic lock: Multiple anomalies detected (10+ in 5 minutes)";
...
export async function emergencyCircuitBreaker() {
  try {
    ...
    if (
      systemControl.circuitBreakerActive ||
      systemControl.integrityLevel === SYSTEM_INTEGRITY_LEVEL.LOCKED
    ) {
      return {
        blocked: true,
        status: 403,
  ...
  try {
    ...
    const updated = await prisma.systemControl.update({
      ...
      data: {
        ...
        integrityLevel: activate
          ? SYSTEM_INTEGRITY_LEVEL.LOCKED
