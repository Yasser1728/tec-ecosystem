...
export async function authMiddleware(req, res, next) {
  try {
    ...
    if (!session) {
      ...
      logger.warn(
        `Unauthorized access attempt to ${req.url} from ${req.headers["x-forwarded-for"] || req.connection.remoteAddress}`,
      );
...
export function requireRole(allowedRoles = []) {
  return async (req, res, next) => {
    try {
      ...
      if (!session) {
        logger.warn(`Unauthorized role check attempt to ${req.url}`);
        return res.status(401).json({
          error: "Unauthorized",
      ...
      if (!allowedRoles.includes(userRole)) {
        logger.warn(
          `Forbidden access attempt by ${session.user?.email} with role ${userRole} to ${req.url}`,
        );
...
export function validateSession(session) {
  ...
  if (now > sessionExpiry) {
    logger.warn("Session expired for user:", { email: session.user?.email });
    return false;
  }
