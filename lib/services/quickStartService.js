...
class QuickStartService {
  ...
  async updateStep(userId, step, data = {}) {
    try {
      const current =
        (await this.getQuickStartStatus(userId)) ||
        (await this.initializeQuickStart(userId));
      ...
      const allStepsComplete = Object.values(QUICK_START_STEPS).every(
        (s) => current.steps[s] === true,
      );
    ...
    try {
      ...
      if (!status) {
        return {
          ...
          description:
            "Sign in or create an account using your Pi Network credentials",
      ...
      if (!steps[QUICK_START_STEPS.PORTFOLIO_CREATION]) {
        return {
          ...
          description:
            "Set up your first asset portfolio to start tracking your wealth",
      ...
      if (!steps[QUICK_START_STEPS.ASSET_ADDITION]) {
        return {
          ...
          description:
            "Add an asset to your portfolio (cryptocurrency, property, investment, etc.)",
          endpoint: "/api/assets",
          method: "POST",
          domain: "assets.pi",
        };
      ...
      if (!steps[QUICK_START_STEPS.INSURANCE_ACTIVATION]) {
        return {
          ...
          description:
            "Review and activate insurance recommendations for your high-value assets",
      ...
      if (!steps[QUICK_START_STEPS.FIRST_INVESTMENT]) {
        return {
          ...
          description:
            "Explore investment opportunities tailored to your risk profile",
      ...
      return {
        ...
        description:
          "You have successfully completed the Quick Start workflow. Explore more features!",
        completed: true,
      };
    } catch (error) {
  ...
  async getProgressPercentage(userId) {
    try {
      ...
      const completedSteps = Object.values(status.steps).filter(
        (s) => s === true,
      ).length;
