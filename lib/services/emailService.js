...
class EmailService {
  constructor() {
    ...
    this.from = process.env.SMTP_FROM || process.env.EMAIL_FROM || 'noreply@tec-ecosystem.com';
  ...
  async sendViaSMTP(emailData) {
    ...
    try {
      ...
      if (error.code === 'ERR_MODULE_NOT_FOUND') {
        ...
        throw new Error('SMTP provider requires nodemailer package. Install with: npm install nodemailer');
      }
      throw error;
    }
  }
  ...
  async sendViaAWSSES(emailData) {
    ...
    try {
      ...
      if (error.code === 'ERR_MODULE_NOT_FOUND') {
        ...
        console.warn('[EmailService] AWS SES: @aws-sdk/client-ses not installed. Falling back to console output.');
        console.warn('[EmailService] To enable AWS SES, run: npm install @aws-sdk/client-ses');
   ...
   */
  async sendSovereignNotification({ to, operationType, domain, operationData, approvalResult, user }) {
