...
export function validateEnvironment() {
  ...
  if (missing.length > 0) {
    ...
    throw new Error('Required environment variables are not set. Check env.local.example for reference.');
  }
...
export function validateNoSecretsExposed(publicEnv = {}) {
  ...
  if (exposed.length > 0) {
    console.error('⚠️  WARNING: Potentially sensitive environment variables exposed:');
