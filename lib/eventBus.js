...
class EventBus extends EventEmitter {
  ...
  subscribe(eventType, handler, options = {}) {
    ...
    const wrappedHandler = async (eventData, metadata) => {
      try {
        ...
        logger.error(`[EventBus] Error in handler for ${eventType}:`, { error });
        this.emit('error', {
          eventType,
          error: error.message,
          domain,
    ...
    logger.info(`[EventBus] Subscribed: ${domain || 'Unknown'} to ${eventType}`);
  ...
  /**
   * Generate unique event ID using cryptographically secure random values
   * 
   * @returns {string} Event ID
   */
  generateEventId() {
    const randomBytes = crypto.randomBytes(16).toString('hex');
    return `evt_${randomBytes}`;
  }
