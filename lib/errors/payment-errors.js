 ...
 * 
 * Note: This function uses string matching for error detection which is a pragmatic approach
 * for catching common error patterns. For more robust error handling, consider:
 * 1. Using custom error classes that extend Error with specific error types
 * 2. Adding error codes to all thrown errors throughout the codebase
 * 3. Using error.name property for error classification
 * 
...
export function handlePaymentError(error, operation, locale = 'en', isDevelopment = false) {
  ...
  // Check for custom error code property first (more reliable)
  let code = error.code || PAYMENT_ERROR_CODES.UNKNOWN_ERROR;
  
  // If no error code, fallback to string matching
  if (code === PAYMENT_ERROR_CODES.UNKNOWN_ERROR) {
    const errorMessage = error.message.toLowerCase();
    
    if (errorMessage.includes('timeout') || errorMessage.includes('timed out')) {
      code = PAYMENT_ERROR_CODES.TIMEOUT;
    } else if (errorMessage.includes('database') || errorMessage.includes('prisma') || errorMessage.includes('db')) {
      // Check database errors before network errors (prisma connection error should be database, not network)
      code = PAYMENT_ERROR_CODES.DATABASE_ERROR;
    } else if (errorMessage.includes('unauthorized') || errorMessage.includes('authentication') || errorMessage.includes('not authenticated')) {
