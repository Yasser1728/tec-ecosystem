...
class PiSDK {
  ...
  async init() {
    ...
    logger.info(`Pi SDK initialized (sandbox: ${sandbox})`);
    return true;
  }
  ...
  async authenticate(scopes = ["username", "payments"]) {
    ...
    try {
      const auth = await window.Pi.authenticate(scopes, (payment) => {
        logger.info("Incomplete payment:", { payment });
      });
      ...
      const payment = await window.Pi.createPayment(
        ...
        },
        {
          onReadyForServerApproval: (id) =>
            this.approvePayment(id, paymentRecord.id),
          onReadyForServerCompletion: (id, tx) =>
            this.completePayment(id, tx, paymentRecord.id),
