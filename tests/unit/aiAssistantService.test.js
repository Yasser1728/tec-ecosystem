...
describe("sanitizePathComponent", () => {
  describe("valid inputs", () => {
    ...
    test("should accept complex valid names", () => {
      expect(sanitizePathComponent("my-file_name.123", "test")).toBe(
        "my-file_name.123",
      );
    });
  });
  ...
  describe("invalid inputs - path traversal attempts", () => {
    ...
    test("should reject path with ..\\", () => {
      expect(() =>
        sanitizePathComponent("..\\windows\\system32", "test"),
      ).toThrow("Invalid test: contains unsafe characters");
    });
    ...
    test("should reject Windows absolute paths", () => {
      expect(() =>
        sanitizePathComponent("C:\\Windows\\System32", "test"),
      ).toThrow("Invalid test: contains unsafe characters");
    });
  });
...
describe("AIAssistantService path security", () => {
  ...
  describe("loadPrompt", () => {
    ...
    test("should reject path traversal in promptId", async () => {
      ...
      await expect(
        aiAssistantService.loadPrompt(maliciousPromptId),
  ...
  describe("loadModelConfig", () => {
    ...
    test("should reject path traversal in modelName", async () => {
      ...
      await expect(
        aiAssistantService.loadModelConfig(maliciousModelName),
  ...
  describe("exportAssistantData", () => {
    ...
    test("should sanitize exportFormat before using in path.join", async () => {
      ...
      await expect(
        aiAssistantService.exportAssistantData(
          validAssistantId,
          maliciousFormat,
        ),
  ...
  describe("loadAssistant", () => {
    ...
    test("should reject path traversal in name", async () => {
      ...
      await expect(
        aiAssistantService.loadAssistant(maliciousName),
  ...
  describe("saveAssistant", () => {
    test("should sanitize name before using in path.join", async () => {
      ...
      await aiAssistantService.saveAssistant(validName, {
        name: "my-assistant",
      });
  ...
  describe("deleteAssistant", () => {
    ...
    test("should reject path traversal in name", async () => {
      ...
      await expect(
        aiAssistantService.deleteAssistant(maliciousName),
      ).rejects.toThrow("Invalid name: contains unsafe characters");
  ...
  describe("loadTemplate", () => {
    ...
    test("should reject path traversal in templateName", async () => {
      ...
      await expect(
        aiAssistantService.loadTemplate(maliciousTemplateName),
  ...
  describe("saveTemplate", () => {
    test("should sanitize templateName before using in path.join", async () => {
      ...
      await aiAssistantService.saveTemplate(validTemplateName, {
        template: "content",
      });
  ...
  describe("importAssistantData", () => {
    ...
    test("should reject path traversal in filename", async () => {
      ...
      await expect(
        aiAssistantService.importAssistantData(maliciousFilename),
      ).rejects.toThrow("Invalid filename: contains unsafe characters");
