  ...
  try {
    ...
    if (!fs.existsSync(servicePath)) {
      const template = [
        ...
        '    return { ok: true, content: taskPrompt, usage: { total_tokens: 0 }, meta: { domain: "' +
          domain +
          '", sandbox: true, role: "PRIMARY" } };',
        "}",
      ].join("\n");
    ...
    const module = await import(
      path.join(CONFIG.servicesFolder, `${domain}.js`)
    );
  ...
  for (const domain of CONFIG.domains) {
    ...
    try {
      ...
      if (getCostSignal().isLowBalance) {
        console.warn(
          `Budget threshold reached for ${domain}. Switching to reserve mode.`,
        );
      }
