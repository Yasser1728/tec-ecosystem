...
class AssetService {
  ...
  async updateAsset(assetId, updates) {
    try {
      ...
      if (
        ...
        const quantity =
          updates.quantity !== undefined ? updates.quantity : asset.quantity;
        const currentPrice =
          updates.currentPrice !== undefined
  ...
  async createTransaction(data) {
    try {
      const totalAmount =
        data.totalAmount || this.calculateValue(data.quantity, data.price);
  ...
  async linkToInsurance(assetId, insuranceData) {
    try {
      ...
      console.log(
        `Asset ${assetId} linked to insurance policy ${insuranceData.policyId}`,
      );
  ...
  async linkToInvestment(assetId, investmentData) {
    try {
      ...
      console.log(
        `Asset ${assetId} linked to investment ${investmentData.investmentId}`,
      );
  ...
  async analyzeAssetData(assetId, options = {}) {
    try {
      ...
      const transactionAnalysis = this.analyzeTransactionPatterns(
        asset.transactions || [],
      );
      ...
      // Investment insights
      const insights = this.generateInvestmentInsights(
        asset,
        performance,
        priceAnalysis,
      );
  ...
  analyzePriceTrends(valuations) {
    ...
    const sortedValuations = [...valuations].sort(
      (a, b) => new Date(a.valuationDate) - new Date(b.valuationDate),
    );
    ...
    const sma7 =
      prices.length >= 7
        ? prices.slice(-7).reduce((a, b) => a + b, 0) / 7
        : null;
  ...
  analyzeTransactionPatterns(transactions) {
    ...
    const totalBought = buyTransactions.reduce(
      (sum, t) => sum + parseFloat(t.quantity),
      0,
    );
    ...
    const avgBuyPrice =
      buyTransactions.length > 0
        ? buyTransactions.reduce((sum, t) => sum + parseFloat(t.price), 0) /
          buyTransactions.length
        : 0;

    const avgSellPrice =
      sellTransactions.length > 0
        ? sellTransactions.reduce((sum, t) => sum + parseFloat(t.price), 0) /
    ...
    return {
      ...
      prices: {
        ...
        averageSellPrice:
          avgSellPrice > 0 ? parseFloat(avgSellPrice.toFixed(8)) : null,
      },
      dividends: {
  ...
  calculateRiskMetrics(valuations, asset) {
    if (!valuations || valuations.length < 10) {
      return {
        riskLevel: "UNKNOWN",
        message:
          "Insufficient data for risk assessment (need at least 10 valuations)",
      };
    }
    ...
    return {
      ...
      sharpeRatio:
        meanReturn > 0 ? parseFloat((meanReturn / stdDev).toFixed(2)) : null,
      metadata: {
        dataPoints: prices.length,
