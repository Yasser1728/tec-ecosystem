...
export class DomainBootstrap {
   ...
   */
  async executeWithControls(operationType, operationData, user, request, executor) {
    // Step 1: Log operation
    const logResult = await this.logOperation(operationType, operationData, user, request);
    
    // Step 2: Request approval
    const approvalResult = await this.requestApproval(operationType, operationData, user, request);
    ...
    try {
      ...
      await this.logOperation(`${operationType}_success`, {
        ...operationData,
      ...
      await this.logOperation(`${operationType}_failed`, {
        ...operationData,
        error: error.message
      }, user, request);
