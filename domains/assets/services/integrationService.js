...
class AssetsIntegrationService {
  ...
  initialize() {
    console.log(
      "[AssetsIntegration] Initializing cross-domain event subscriptions...",
    );
  ...
  subscribeFundXEvents() {
    ...
    const unsubInvestmentCreated = eventBus.subscribe(
      ...
      async (eventData, metadata) => {
        console.log(
          "[AssetsIntegration] Received fundx.investment.created event",
        );
        try {
          const asset =
            await this.assetService.handleFundXInvestment(eventData);
          ...
          console.log(
            `[AssetsIntegration] Created asset ${asset.id} from FundX investment`,
          );
        } catch (error) {
          console.error(
            "[AssetsIntegration] Error handling FundX investment:",
            error,
          );
          throw error;
        }
    ...
    const unsubInvestmentUpdated = eventBus.subscribe(
      ...
      async (eventData, metadata) => {
        console.log(
          "[AssetsIntegration] Received fundx.investment.updated event",
        );
        try {
          ...
          if (linkedAsset) {
            ...
            console.log(
              `[AssetsIntegration] Updated asset ${linkedAsset.id} from FundX investment update`,
            );
          }
        } catch (error) {
          console.error(
            "[AssetsIntegration] Error handling FundX investment update:",
            error,
          );
        }
      },
  ...
  subscribeEstateEvents() {
    const unsubPropertyPurchased = eventBus.subscribe(
      ...
      async (eventData, metadata) => {
        console.log(
          "[AssetsIntegration] Received estate.property.purchased event",
        );
        try {
          ...
          console.log(
            `[AssetsIntegration] Created asset ${asset.id} from Estate property purchase`,
          );
        } catch (error) {
          console.error(
            "[AssetsIntegration] Error handling Estate property purchase:",
            error,
          );
          throw error;
        }
  ...
  subscribeCommerceEvents() {
    const unsubProductPurchased = eventBus.subscribe(
      ...
      async (eventData, metadata) => {
        console.log(
          "[AssetsIntegration] Received commerce.product.purchased event",
        );

        // Only process if marked for asset tracking
        if (!eventData.trackAsAsset) {
          console.log(
            "[AssetsIntegration] Product not marked for asset tracking, skipping",
          );
          return;
        }
        ...
        try {
          const asset =
            await this.assetService.handleCommerceProduct(eventData);
          ...
          if (asset.success !== false) {
            ...
            console.log(
              `[AssetsIntegration] Created asset ${asset.id} from Commerce product purchase`,
            );
          }
        } catch (error) {
          console.error(
            "[AssetsIntegration] Error handling Commerce product purchase:",
            error,
          );
          throw error;
        }
  ...
  subscribeInsureEvents() {
    ...
    const unsubPolicyLinked = eventBus.subscribe(
      ...
      async (eventData, metadata) => {
        console.log(
          "[AssetsIntegration] Received insure.policy.asset.linked event",
        );
        try {
          ...
          console.log(
            `[AssetsIntegration] Linked asset ${eventData.assetId} to insurance policy ${eventData.policyId}`,
          );
        } catch (error) {
          console.error(
            "[AssetsIntegration] Error linking insurance to asset:",
            error,
          );
          throw error;
        }
  ...
  publishPortfolioValueUpdated(portfolioId, userId, oldValue, newValue) {
    ...
    eventBus.publish(
      "assets.portfolio.value.updated",
      {
        portfolioId,
        userId,
        oldValue,
