...
class InvestmentService {
  ...
  publishInvestmentCreated(investment, userId, strategy) {
    ...
    console.log(
      `[FundX] Published event: fundx.investment.created for ${investment.id}`,
    );
  }
    ...
    eventBus.subscribe(
      ...
      async (eventData, metadata) => {
        ...
        try {
          ...
          console.log(
            `[Assets] Created asset ${asset.id} from FundX investment ${eventData.investmentId}`,
          );
...
class AnalyticsIntegrationService {
  ...
  async updateUserFinancialProfile(eventData, metadata) {
    console.log(
      "[Analytics] Updating financial profile for user:",
      eventData.userId,
    );
    ...
    console.log(
      "[Analytics] Financial profile updated, insights:",
      insights.length,
    );
  }
  ...
  async updatePortfolioValue(portfolioId, value) {
    console.log(
      `[Analytics] Updating portfolio ${portfolioId} value by ${value}`,
    );
  }
...
class AlertIntegrationService {
  ...
  async createDefaultAlerts(eventData, metadata) {
    ...
    if (eventData.assetType === "INVESTMENT") {
      ...
      console.log(
        `[Alert] Created default alerts for asset ${eventData.assetId}`,
      );
    }
  }
